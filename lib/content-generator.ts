...
function buildPrompt(context: GenerationContext) {
  ...
  const contentTypeAngles: Record<string, string[]> = {
    ...
    "驾校": [
      "从距离、练车排队和接送方式三个维度比较，不做绝对推荐",
      "写报名前必须问清楚的费用和时间问题，避免后续加价",
      "结合课程与考试安排，给出适合大学生的练车节奏"
    ],
    "二手书": [
      "按课程需要区分必买、可借和不建议提前买的教材",
      "说明校内二手渠道、版本核对和合理价格判断",
      "用一学期真实使用频率来判断是否值得购买"
    ],
    "兼职": [
      "优先讲信息核验、收费陷阱和时间成本，不承诺收入",
      "按校内、线上、周末三种场景分析适合人群",
      "提醒保护个人信息，并给出遇到可疑招聘时的处理动作"
    ]
  };
